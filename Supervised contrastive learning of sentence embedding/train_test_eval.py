...
def eval_in_train(model,Config,tk):
    ...
    def batcher(params, batch):#在sts里面使用用于编码数据
        ...
        for sentence in sentences:
            sentence_id = tk.encode(sentence, max_length=Config.enc_maxlen, truncation=True)#.replace("\"","")
        ...
        with torch.no_grad():
            if Config.multi_gpu==True:
                output=model.module.Paraphraser(model.module.get_vecs(input_ids,attention_masks))#
        return output.cpu()
